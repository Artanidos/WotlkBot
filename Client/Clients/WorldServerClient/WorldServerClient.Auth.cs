...
namespace WotlkClient.Clients
{
    partial class WorldServerClient
    {
        ...
        public void CharEnumRequest(CharEnumCompletedCallBack callback)
        {
            charEnumCompletedCallBack = callback;
        ...
        public void HandleCharEnum(PacketIn packet)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                packet.ReadUInt32();
                packet.ReadUInt32();
                packet.ReadBytes(9 * 20 + 31);
            }

            Charlist = characterList;

            if (charEnumCompletedCallBack != null)
                charEnumCompletedCallBack(0);   
        }

        public void LoginPlayer(Character chr, CharLoginCompletedCallBack callback)
        {
            charLoginCompletedCallBack = callback;
        ...
        public void DoAuthSession()
        {
            try
            {
                ...
                packet.Write((UInt32)0);    // region

                packet.Write((UInt32)0);    // battlegroupid
                packet.Write((UInt32)1);    // Realmid
                packet.Write((UInt64)2);    // Dosresponse
                ...
                if (WantToCrash)
                {
                    ...
                    byte[] buffer2 = WotlkClient.Shared.Compression.Compress(addons);
